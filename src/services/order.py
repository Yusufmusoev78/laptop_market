...
class OrderService:
    ...
    async def create_order(self, db: AsyncSession, order_in: OrderCreate, buyer: User) -> Order:
        ...
        notification = {
            "type": "order_created",
            "message": f"New order placed by {buyer.username or buyer.email}: {order_in.quantity}x {laptop.brand} {laptop.model_name}"
        }
        await manager.broadcast_to_admins(notification)
        if laptop.owner_id is not None:
            await manager.send_to_user(laptop.owner_id, notification)
